...
const Paiement = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/renouvellement/step5/page5";
  ...
  const handlePrevious = () => {
    if (idProc) {
      ...
      router.push('/operateur/renouvellement/step4/page4');
    }
  };
  ...
  const handlePayment = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/renouvellement/step5/confirmationpay?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    }
  };
  ...
  const handleCardRedirect = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/renouvellement/step5/paiementadahabia?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
      return;
    }
    router.push('/operateur/renouvellement/step5/paiementadahabia');
  };
            ...
            <div className={styles.container}>
              ...
              {/*<button
                ...
                onClick={() =>
                  idProc
                    ? router.push(
                        `/operateur/renouvellement/step4/page4?id=${idProc}`,
                      )
                    : router.push('/operateur/renouvellement/step4/page4')
                }
                type="button"
