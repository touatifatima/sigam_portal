...
const Facture = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/renouvellement/step4/page4";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(`/operateur/renouvellement/step3/page3`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/renouvellement/step5/page5?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/renouvellement/step4/page4</p>
        )}
      </div>
    );
  }
