...
const Facture = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/cession/step3/page3";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(
        `/operateur/cession/step2/page2?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } else {
      router.push(`/operateur/cession/step2/page2`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/cession/step4/page4?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/cession/step3/page3</p>
        )}
      </div>
    );
  }
