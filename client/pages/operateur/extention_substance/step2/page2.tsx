...
const Facture = () => {
  ...
  const extractLocationFromPayload = (payload: any) => {
    ...
    return { commune, daira, wilaya, communes, dairas, wilayas };
  };
      ...
      Object.entries(next).forEach(([key, value]) => {
        if (value !== null && value !== undefined && value !== "") {
          if (
            ...
            (cleaned as any)[key] = mergeUniqueStrings((base as any)?.[key], value as string[]);
            return;
          }
  ...
  useEffect(() => {
    ...
    const fetchDemandeData = async () => {
      ...
      try {
        ...
        const payloadPermisId =
          Number(payload?.id_permis ?? payload?.demande?.id_permis ?? payload?.idPermis);
        if (Number.isFinite(payloadPermisId)) {
          setDetectedPermisId(payloadPermisId);
        }
        const societe = resolveTitulaire(payload);
        const { commune, daira, wilaya, communes, dairas, wilayas } =
          extractLocationFromPayload(payload);
  ...
  useEffect(() => {
    if (!apiURL || !resolvedPermisId) return;
    const controller = new AbortController();
    const loadPermisBase = async () => {
      try {
        ...
        const locationPayload = {
          commune:
            pickName(payload?.localisation_officielle?.commune, [
              "nom_communeFR",
              "nom_commune",
              "lib_commune",
              "nom",
          ...
          daira:
            pickName(payload?.localisation_officielle?.daira, [
              "nom_dairaFR",
              "nom_daira",
              "lib_daira",
              "nom",
          ...
          wilaya:
            pickName(payload?.localisation_officielle?.wilaya, [
              "nom_wilayaFR",
              "nom_wilaya",
              "lib_wilaya",
              "nom",
            ]) ?? null,
          communes: extractNames(payload?.communes, [
            "nom_communeFR",
            "nom_commune",
            "lib_commune",
            "nom",
          ]),
          dairas: extractNames(payload?.dairas, [
            "nom_dairaFR",
            "nom_daira",
            "lib_daira",
            "nom",
          ]),
          wilayas: extractNames(payload?.wilayas, [
            "nom_wilayaFR",
            "nom_wilaya",
            "lib_wilaya",
            "nom",
        ...
        const permitSubstances = Array.isArray(payload?.substances_officielles)
        ...
        mergeDemandeInfo({
          typePermis:
            ...
            payload?.typePermis?.code_type ??
            null,
          commune: locationPayload.commune,
          daira: locationPayload.daira,
          wilaya: locationPayload.wilaya,
          communes: locationPayload.communes,
          dairas: locationPayload.dairas,
          wilayas: locationPayload.wilayas,
          superficie,
          substances: permitSubstances,
        });
      } catch (error) {
        if (!axios.isCancel(error)) {
          console.warn("[Facture] permis fallback fetch failed", error);
        }
      }
    };
    loadPermisBase();
    return () => controller.abort();
  }, [apiURL, resolvedPermisId]);
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/extention_substance/step2/page2";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(
        `/operateur/extention_substance/step1/page1?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } else {
      router.push(`/operateur/extention_substance/step1/page1`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    if (!etapeId) {
      setConfirmMessage(
        "Étape introuvable. Vérifiez le page_route en base (step2/page2).",
      );
      return;
    }
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/extention_substance/step3/page3?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/extention_substance/step2/page2</p>
        )}
      </div>
    );
  }
