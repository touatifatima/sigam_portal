...
const Facture = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/fusion_permis/step4/page4";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(`/operateur/fusion_permis/step3/page3`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/fusion_permis/step5/page5?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/fusion_permis/step4/page4</p>
        )}
      </div>
    );
  }
