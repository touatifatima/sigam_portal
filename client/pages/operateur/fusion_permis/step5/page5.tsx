...
const Paiement = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/fusion_permis/step5/page5";
  ...
  const handlePrevious = () => {
    if (idProc) {
      ...
      router.push(
        `/operateur/fusion_permis/step4/page4?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } else {
      router.push('/operateur/fusion_permis/step4/page4');
    }
  };
  ...
  const handleSaveEtape = async () => {
    ...
    if (!etapeId) {
      setEtapeMessage(
        "Étape introuvable. Vérifiez le page_route en base (step5/page5).",
      );
      return false;
    }
    setSavingEtape(true);
  ...
  const handlePayment = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/fusion_permis/step5/confirmationpay?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    }
  };
  ...
  const handleCardRedirect = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/fusion_permis/step5/paiementadahabia?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
      return;
    }
    router.push('/operateur/fusion_permis/step5/paiementadahabia');
  };
            ...
            <div className={styles.container}>
              ...
              {/*<button
                ...
                onClick={() =>
                  idProc
                    ? router.push(
                        `/operateur/fusion_permis/step4/page4?id=${idProc}`,
                      )
                    : router.push('/operateur/fusion_permis/step4/page4')
                }
                type="button"
