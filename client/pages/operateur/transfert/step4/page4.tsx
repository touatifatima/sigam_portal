...
const Facture = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/transfert/step4/page4";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(
        `/operateur/transfert/step3/page3?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } else {
      router.push(`/operateur/transfert/step3/page3`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    if (!etapeId) {
      setConfirmMessage(
        "Étape introuvable. Vérifiez le page_route en base (step4/page4).",
      );
      return;
    }
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/transfert/step5/page5?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/transfert/step4/page4</p>
        )}
      </div>
    );
  }
