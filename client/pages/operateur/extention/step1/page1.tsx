...
export default function CadastrePage() {
  ...
  const isOriginalPoint = useCallback(
    (pointId: number) => points.some((p) => p.id === pointId && p.isOriginal),
    [points],
  );
  const canEditPoint = useCallback(
    (pointId: number) => !lockPerimeter && !isOriginalPoint(pointId),
    [lockPerimeter, isOriginalPoint],
  );
  const canDeletePoint = useCallback(
    (pointId: number) => !lockPerimeter && !isOriginalPoint(pointId) && points.length > 3,
    [lockPerimeter, isOriginalPoint, points.length],
  );
  const hasOriginalPoints = useMemo(() => points.some((p) => p.isOriginal), [points]);
  ...
  const handleTitreSelected = useCallback(
    async (attrs: any) => {
      ...
      try {
        ...
        console.error('Failed to load GUNAM procedure history for permis', permisCode, e);
        setExistingPolygons([]);
        setHistoryProcedures([]);
        setSelectedHistoryProcId(null);
      }
    ...
    const loadRenewalPerimeter = async () => {
      try {
        const res = await axios.get(
          `${apiURL}/api/procedures/extension/perimetre/latest`,
  ...
  const { phases: stepperPhases, etapeIdForRoute } = useStepperPhases(
    procedureData,
    apiURL,
    'operateur/extention/step1/page1',
  );
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = 'operateur/extention/step1/page1';
  ...
  const handleExcelPick = () => {
    if (lockPerimeter || hasOriginalPoints) return;
  ...
  const insertPointAfter = (index: number) => {
    ...
    if (base?.id != null && !canEditPoint(base.id)) return;
    const newPoint: Point = {
      id: generateId(),
  ...
  const movePointUp = (index: number) => {
    ...
    if ((current && !canEditPoint(current.id)) || (previous && !canEditPoint(previous.id))) return;
    setPoints((prev) => {
      const next = [...prev];
  ...
  const movePointDown = (index: number) => {
    ...
    if ((current && !canEditPoint(current.id)) || (nextPoint && !canEditPoint(nextPoint.id))) return;
    setPoints((prev) => {
      const next = [...prev];
    ...
    try {
      ...
      if (isRenewal) {
        const res = await axios.post(
          `${apiURL}/api/procedures/extension/${idProc}/perimetres`,
        ...
        setSuccess('Perimetre d extension enregistre.');
        setTimeout(() => setSuccess(null), 3000);
        return true;
      }
 ...
 const handleNext = async () => {
    ...
    try {
      ...
      router.push(`/operateur/extention/step2/page2?id=${idProc}${permisParam}`);
    } catch (err) {
                ...
                <aside className={styles.pointsPanel}>
                  ...
                  <div className={styles.systemSelect}>
                    ...
                    <Select
                      ...
                      disabled={lockPerimeter || hasOriginalPoints}
                    >
                    ...
                    {points.map((point, index) => (
                      <div className={styles.pointRow} key={point.id}>
                        ...
                        <Input
                          ...
                          disabled={!canEditPoint(point.id)}
                        />
                        <Input
                          type="number"
                          placeholder="Y (N)"
                        ...
                        <button
                          ...
                          disabled={!canDeletePoint(point.id)}
                          title={
                            lockPerimeter
                            ...
                            <div className={styles['table-row']} key={point.id}>
                              <div>
                                <input
                                  ...
                                  disabled={!canEditPoint(point.id) || editableRowId !== point.id}
                                  min={29}
                                  max={32}
                                />
                              </div>
