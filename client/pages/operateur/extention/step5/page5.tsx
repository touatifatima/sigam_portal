...
const Paiement = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/extention/step5/page5";
  ...
  const handlePrevious = () => {
    if (idProc) {
      ...
      router.push(
        `/operateur/extention/step4/page4?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } else {
      router.push('/operateur/extention/step4/page4');
    }
  };
  ...
  const handlePayment = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/extention/step5/confirmationpay?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    }
  };
  ...
  const handleCardRedirect = async () => {
    ...
    if (idProc) {
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/extention/step5/paiementadahabia?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
      return;
    }
    router.push('/operateur/extention/step5/paiementadahabia');
  };
            ...
            <div className={styles.container}>
              ...
              {/*<button
                ...
                onClick={() =>
                  idProc
                    ? router.push(
                        `/operateur/extention/step4/page4?id=${idProc}`,
                      )
                    : router.push('/operateur/extention/step4/page4')
                }
                type="button"
