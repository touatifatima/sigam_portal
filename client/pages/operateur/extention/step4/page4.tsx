...
const Facture = () => {
  ...
  const etapeIdForThisPage = useMemo(() => {
    ...
    const pathname = "operateur/extention/step4/page4";
  ...
  const handleBack = () => {
    if (idProc) {
      ...
      router.push(`/operateur/extention/step3/page3`);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const permisParam = searchParams?.get('permisId');
      router.push(
        `/operateur/extention/step5/page5?id=${idProc}${permisParam ? `&permisId=${permisParam}` : ''}`,
      );
    } catch (error) {
    ...
    return (
      <div className={styles.loadingState}>
        ...
        {procedureData && !etapeIdForThisPage && (
          <p>Étape page_route introuvable: operateur/extention/step4/page4</p>
        )}
      </div>
    );
  }
