...
import styles from './mes_procedures.module.css';
...
const toggleDropdown = (demandeId: number) => {
  ...
  const goToEtape = async (idProc: number) => {
    if (idProc === undefined || idProc === null) {
      console.error("Impossible de recuperer l'etape : id_proc est undefined/null");
      return;
    }
    ...
    try {
      ...
      const res = await axios.get(
        `${apiURL}/api/procedure-etape/current/${idProc}`,
        { withCredentials: true },
      );
      const etape = res.data;
      // Fallback link if backend doesn't supply one
      const __fallbackLink =
        typeof etape?.id_etape === 'number'
      ...
      if (etape?.link) {
        ...
        setError("Lien de l'etape introuvable");
      }
    } catch (err) {
      const message = (err as any)?.response?.data?.message || (err as Error).message || 'Erreur inconnue';
      console.error("Erreur lors de la recuperation de l'etape :", message);
      setError("Impossible de recuperer l'etape actuelle");
    } finally {
      setIsLoading(false);
    }
  };
...
{openDropdown === d.id_demande && (
  <div className={styles.dropdownMenu} role="menu" aria-label="Actions menu">
    <button
      ...
      onClick={() => {
        setOpenDropdown(null);
        const pid = d?.procedure?.id_proc;
        if (pid === undefined || pid === null) {
          console.error("Impossible de continuer : id_proc manquant sur la proc�dure");
          toast.error("Proc�dure introuvable (id manquant)");
          return;
        }
        goToEtape(pid);
      }}
