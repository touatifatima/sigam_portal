...
const UserObligationsPage = () => {
  ...
  const paymentTypes = useMemo(() => {
    ...
    obligations.forEach((o) => {
      if (o.typePaiement?.id && o.typePaiement?.libelle) {
        uniques.set(o.typePaiement.id, o.typePaiement.libelle);
      }
    });
    return Array.from(uniques.entries()).map(([id, libelle]) => ({ id, libelle }));
  }, [obligations]);

  const filteredObligations = useMemo(() => {
    if (paymentTypeFilter === 'all') return obligations;
    const targetId = Number(paymentTypeFilter);
    return obligations.filter((o) => o.typePaiement?.id === targetId);
  }, [obligations, paymentTypeFilter]);
  ...
  useEffect(() => {
    const isPaid = (status: string) => {
      ...
      return s === 'payé' || s === 'payé' || s === 'paye' || s === 'pay‚';
    };

    const total = filteredObligations.reduce((sum: number, obligation: Obligation) => {
      return sum + (obligation.amount || 0);
    }, 0);
    
    const paid = filteredObligations.reduce((sum: number, obligation: Obligation) => {
      if (isPaid(obligation.status)) {
        return sum + (obligation.amount || 0);
      }
      return sum;
    }, 0);
    
    const pending = filteredObligations.reduce((sum: number, obligation: Obligation) => {
      if (!isPaid(obligation.status)) {
        return sum + (obligation.amount || 0);
      }
      return sum;
...
useEffect(() => {
  ...
  const getStatusText = (status: string) => {
    const s = (status || '').toLowerCase();
    if (s === 'payé' || s === 'payé' || s === 'paye' || s === 'pay‚') return 'Payé';
    if (s === 'en retard') return 'En retard';
    if (s === 'a payer' || s === 'à payer') return 'En attente';
    return status;
  };
              ...
              <div className={styles.container}>
                <div className={styles.header}>
                  ...
                  <p className={styles.subtitle}>
                    ...
                    {permisDetails?.code_permis && (
                      <> <strong>Code: {permisDetails.code_permis} {permisDetails.typePermis.code_type}</strong></>
                    )}
                  </p>
                </div>
                ...
                <div className={styles.obligationsSection}>
                  ...
                  <div className={styles.obligationsTableContainer}>
                    <table className={styles.obligationsTable}>
                      ...
                      <tbody>
                        {(filteredObligations || []).map((obligation, idx) => (
                          <tr
                            key={obligation.id}
                            className={`${styles.obligationRow} ${
                              ((obligation.status || '').toLowerCase() === 'payé' ||
                                (obligation.status || '').toLowerCase() === 'payé' ||
                                (obligation.status || '').toLowerCase() === 'paye' ||
                                (obligation.status || '').toLowerCase() === 'pay‚')
                                ? styles.rowPaid
                                : ''
                            }`}
                          >
