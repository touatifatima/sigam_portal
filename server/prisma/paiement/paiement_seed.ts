...
export async function main() {
  const csvFilePath = path.join('T:', 'Amina', 'BaseSicma_Urgence', 'df_paiement.csv');
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.PaiementCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('data', (row: any) => {
      ...
      rows.push({
        ...
        id_obligation: parseId(row.id_obligation) || null,
        montant_paye: parseFloat(row.montant_paye),
        date_paiement: parseDate(row.date_paiement)!,
        num_quittance: row.num_quitt || null,
        date_remisOp: parseDate(row.date_remisOp),
        etat_paiement: row.etat_paiement || null,
        num_perc: row.NumeroPerc || null,
      });
    })
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.paiement.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id != null) {
