...
export async function main() {
  ...
  const csvFilePath = path.join('T:', 'Amina', 'BaseSicma_Urgence', 'df_substances.csv');
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on("data", (row: SubstancesCSV) => {
      ...
      const nomFR = (row.nom_subFR || '').trim();
      const nomAR = (row.nom_subAR || '').trim();
      const cat = (row.categorie_sub || '').trim();
      const idRedev = row.id_redevance && row.id_redevance.trim() !== '' ? Number(row.id_redevance.trim()) : null;
