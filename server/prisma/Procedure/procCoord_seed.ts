...
export async function main() {
  const csvFilePath = path.join('T:', 'Amina', 'BaseSicma_Urgence', 'df_procedureCoord.csv');
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.ProcedureCoordCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('data', (row: any) => {
      totalRows++;
      rows.push({
        id_procedureCoord: parseInt(row.Id_procCoord, 10),
        id_proc: parseInt(row.id_proc, 10),
        id_coordonnees: parseInt(row.id_coord, 10),
        statut_coord: 'NOUVEAU', // default required enum (StatutCoord)
      });
    })
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.procedureCoord.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_procedureCoord != null) {
            skippedIds.push(row.id_procedureCoord);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_procedureCoord ?? 'unknown'} due to error: ${error.message}`);
        }
      }

      console.log(`Insertion finished. ✅ Total skipped rows: ${skippedRows}`);
      if (skippedIds.length > 0) {
        console.log("IDs des procCoord non insérées:", skippedIds.join(", "));
      }
