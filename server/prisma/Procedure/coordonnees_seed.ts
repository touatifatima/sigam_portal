...
export async function main() {
  const csvFilePath = path.join('T:', 'Amina', 'BaseSicma_Urgence', 'df_coordonneesGeneral.csv');
  let totalRows = 0;
  let skippedRows = 0;
  const skippedIds: number[] = [];

  const rows: Prisma.CoordonneePortailCreateManyInput[] = [];
  ...
  fs.createReadStream(csvFilePath)
    ...
    .on('end', async () => {
      ...
      for (const row of rows) {
        try {
          await prisma.coordonneePortail.create({ data: row });
        } catch (error: any) {
          skippedRows++;
          if (row.id_coordonnees != null) {
            skippedIds.push(row.id_coordonnees);
          }
          console.warn(`⚠️ Skipped demande id=${row.id_coordonnees ?? 'unknown'} due to error: ${error.message}`);
        }
      }
