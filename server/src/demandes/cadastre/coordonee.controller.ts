...
export class CoordonneesController {
  ...
  async getCoordonneesByProcedure(@Param('id') id_proc: string) {
    try {
      const coords = await this.coordonneesService.getCoordonneesByProcedure(
        Number(id_proc),
      );
      return coords;
    } catch (error) {
  ...
  async deleteCoordonneesByProcedure(@Param('id') id_proc: string) {
    try {
      return await this.coordonneesService.deleteCoordonneesByProcedure(
        Number(id_proc),
      );
    } catch (error) {
      throw new HttpException(
  ...
  @Get('/permis/:id')
  async getCoordonneesByPermis(@Param('id') id_permis: string) {
    const coords = await this.coordonneesService.getCoordonneesByPermis(
      Number(id_permis),
    );
    return coords;
  }

  @Get('/permis-code/:code')
  async getPerimetersHistoryByPermisCode(@Param('code') code: string) {
    try {
      return await this.coordonneesService.getPerimeterHistoryByPermisCode(code);
    } catch (error) {
      throw new HttpException(
        'Failed to fetch permit history by code',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
