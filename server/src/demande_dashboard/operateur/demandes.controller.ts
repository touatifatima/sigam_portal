...
@Get('mine')
async mine(
  @Query('utilisateurId') utilisateurId?: number,
  @Query('responsable') responsable?: string, // Garde pour compatibilité ancienne version
) {
  // Priorité à utilisateurId (nouvelle méthode fiable)
  if (utilisateurId) {
    return this.service.findByUtilisateurId(utilisateurId);
  }

  // Fallback ancien mode (si responsable est envoyé)
  if (responsable) {
    return this.service.findByResponsable(responsable);
  }

  throw new BadRequestException('utilisateurId ou responsable requis');
}
  @Get(':id')
