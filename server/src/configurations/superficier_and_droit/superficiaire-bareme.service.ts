...
export class SuperficiaireBaremeService {
  ...
  async create(createDto: CreateSuperficiaireBaremeDto) {
    try {
      ...
      throw new BadRequestException({
        ...
        details: error.meta || error.message || error,
      });
    }
  }
  ...
  async findOne(id: number) {
    ...
    if (!entry) {
      throw new NotFoundException(
        `SuperficiaireBareme with ID ${id} not found`,
      );
    }
  ...
  async remove(id: number) {
    try {
      ...
      if (typePermisIds.length > 0) {
        const [associatedDemandes, associatedDossiers, associatedPermis] =
          await Promise.all([
            this.prisma.demandePortail.count({
              where: { id_typePermis: { in: typePermisIds } },
            }),
            this.prisma.dossierAdministratif.count({
              where: { id_typePermis: { in: typePermisIds } },
            }),
        ...
        if (
          associatedDemandes > 0 ||
          associatedDossiers > 0 ||
          associatedPermis > 0
        ) {
          throw new BadRequestException(
